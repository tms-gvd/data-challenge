...
class ConfigParser:
    def __init__(self, config, resume=None, with_wandb=False, modification=None):
        ...
        # TODO: set save_dir where trained model and log will be saved.
        if self.with_wandb:
            self._save_dir = Path(wandb.run.dir)
        else:
            self._save_dir = None
    ...
    def get_logger(self, config):
        if config.with_wandb:
            print("Logging to wandb")
            wandb.init(project="in-queue", entity="data-challenge", config=config)
        else:
            print("No logging")
            wandb.init(mode="disabled")
