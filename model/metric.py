...
class PrecisionPerClass(object):
    ...
    def _precision_per_class(self, output, target):
        NUM_CLASSES = os.environ.get("NUM_CLASSES", 37)
        with torch.no_grad():
            pred = torch.argmax(output, dim=1)
            assert pred.shape[0] == len(target)
            preds = []
            for i in range(int(NUM_CLASSES)):
                preds.append(
                    [(pred == i).logical_and(target == i).sum(), (pred == i).sum()]
                )
...
class IoU(object):
    ...
    def _iou_interval(
        self, interval1: Tuple[float, float], interval2: Tuple[float, float]
    ) -> float:
        """
    ...
    def _iou_sequence(self, pred_annotations, true_annotations):
        ...
        for i in range(len(true_annotations) - 1):
            total_iou += self._iou_interval(
                pred_annotations[i : i + 2], true_annotations[i : i + 2]
            )
        mean_iou = (
            total_iou / (len(true_annotations) - 1)
            if len(true_annotations) > 1
            else 0.0
        )
        return mean_iou
    ...
    def __call__(self, prediction: np.ndarray, target: np.ndarray) -> float:
        ...
        assert len(prediction) == len(
            target
        ), "predictions and target arrays should have the same length !"
